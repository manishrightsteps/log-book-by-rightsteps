  ...
  try {
    ...
    const changelogs = files
      ...
      .map(file => {
        ...
        // Extract date from content - handle multiple formats
        const dateMatch = content.match(/\((\d{4}-\d{2}-\d{2})\)/) || 
                         content.match(/\*\*Date:\*\*\s*([A-Za-z]+ \d{1,2}, \d{4})/);
        let date = null;
        
        if (dateMatch) {
          if (dateMatch[1].includes('-')) {
            date = dateMatch[1]; // Already in YYYY-MM-DD format
          } else {
            // Convert "August 25, 2025" to "2025-08-25"
            try {
              const parsedDate = new Date(dateMatch[1]);
              date = parsedDate.toISOString().split('T')[0];
            } catch (e) {
              date = dateMatch[1]; // Keep original if parsing fails
            }
          }
        }
