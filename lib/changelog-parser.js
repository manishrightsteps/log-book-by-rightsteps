...
export function parseChangelogContent(content) {
  ...
  while ((match = sectionRegex.exec(content)) !== null) {
    ...
    while ((subsectionMatch = subsectionRegex.exec(sectionContent)) !== null) {
      ...
      // Parse items from this subsection
      const items = subsectionContent
        .split('\n')
        .filter(line => line.trim().startsWith('- [x]') || line.trim().startsWith('-'))
        .map(line => {
          // Remove checkbox and dash, clean up
          return line.replace(/^-\s*\[x\]\s*/, '').replace(/^-\s*/, '').trim();
        })
    ...
    if (subsectionItems.length > 0) {
      ...
      // Otherwise, parse the section directly for items
